...
它会串行执行依赖审计、Lint、类型检查、全量测试、生产构建、加密恢复演练和生产商业化门禁；任一步失败都禁止发布。
...
```bash
npm run repair:media-index
npm run repair:media-index -- --apply
```

生产环境没有 `--apply` 会直接拒绝；执行前应完成数据库备份并核对脚本输出的课程与资产 ID。

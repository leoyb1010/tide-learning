...
/* ============================================================
   课程卡「视觉面」（纯展示，无导航壳）
   —— 把封面 + 元信息抽成独立面，供两种外壳复用，杜绝视觉分叉：
      · CourseCard（server）：外面套 <Link>，直达详情（相关课程等场景沿用）。
      · CoursePreviewCard（client）：外面套 <button>，先弹预览气泡再进（课程库两段式）。
   纯函数、用 /dist/ssr 图标（客户端组件也可安全引用）；不引任何 server 链。
   ============================================================ */
export function CourseCardFace({ course }: { course: CourseCardData }) {
  const grad = trackGradientVar(course.category ?? "");
  // 封面决策（无法 onError 探测）：有专属封面走 cover-<slug>.jpg，

...
/** 测验块：单选，点击即时判分。选对绿 / 选错红并高亮正确项 + 解析。纯前端 state。 */
function QuizBlock({
  question,
  options,
  answerIndex,
  ...
  return (
    <section className={`${CARD} p-5 sm:p-6`}>
        ...
        {options.map((opt, i) => {
          ...
          return (
            <button
              ...
              disabled={answered}
              onClick={() => setPicked(i)}
              className={`flex items-center justify-between gap-3 rounded-[var(--radius-card-sm)] border px-4 py-3 text-left text-[14px] transition-colors disabled:cursor-default ${cls}`}
            >
      ...
      {answered && (
        <div className="mt-4 rounded-[var(--radius-card-sm)] bg-[var(--surface-inset)] p-4">
...
function KeypointBlock({ points, courseId }: { points: string[]; courseId?: string }) {
  ...
  return (
    <section className="rounded-[var(--radius-card)] border border-[var(--red-soft-border)] bg-[var(--red-soft)] p-5 shadow-[var(--card),var(--inner-hi)] sm:p-6">
      ...
      <ul className="flex flex-col gap-2.5">
        {points.map((p, i) => (
          <li key={i} className="flex items-start gap-2.5 text-[14px] leading-relaxed text-[var(--ink)]">
            <span className="mt-[7px] h-1.5 w-1.5 shrink-0 rounded-full bg-[var(--red)]" />
...
/** 提示框：info 蓝调 / warn 黄调（走 --info/--warn 语义 token）。 */
function CalloutBlock({ tone, markdown }: { tone: "info" | "warn"; markdown: string }) {
  ...
  return (
    <section
      className="flex gap-3 rounded-[var(--radius-card)] border p-4 shadow-[var(--card),var(--inner-hi)] sm:p-5"
...
/** objectives —— 节首页头卡「本节你将学会」，items 逐条 stagger 亮起（✓ 绿点）。 */
          ...
          <li
            ...
            style={
              reduce
                ? undefined
                : {
                    animation: `staggerIn .5s var(--ease-out-expo) both`,
...
/** scene —— 深色渐变「为什么学」故事开场：大字引言 + 场景铺陈。 */
function SceneBlock({ title, markdown }: { title: string; markdown: string }) {
  return (
    <section
      className="relative overflow-hidden rounded-[var(--radius-card)] p-6 shadow-[var(--card)] sm:p-8"
...
/** dialog —— 聊天气泡左右分列（speaker 交替左右），逐条浮现，note 作小字注释。 */
        ...
        {turns.map((t, i) => {
          ...
          return (
            ...
            >
              ...
              <div
                className={`max-w-[85%] rounded-[16px] px-4 py-2.5 text-[14px] leading-relaxed ${
                  isRight
                    ? "rounded-tr-sm bg-[var(--red-soft)] text-[var(--ink)]"
                    : "rounded-tl-sm bg-[var(--surface-inset)] text-[var(--ink)]"
                }`}
              >
                {t.text}
              </div>
...
/** steps —— 竖向步骤条：序号圆点 + 连接线 + 标题 + detail。 */
          ...
          return (
            <li key={i} className="flex gap-3.5">
              ...
              <div className="flex flex-col items-center">
                <span className="inline-flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-[var(--red)] text-[12px] font-bold text-white shadow-[var(--card)]">
                  {i + 1}
                </span>
                {!last && <span className="w-px flex-1 bg-[var(--border2)]" aria-hidden />}
              </div>
              {/* 内容 */}
...
/** compare —— 双栏对比：left 误区调（warn）vs right 正确调（ok），各列 items。 */
  ...
  return (
    <section className={`${CARD} p-5 sm:p-6`}>
      ...
      <div className="grid gap-3 sm:grid-cols-2">
        <CompareColumn variant="warn" heading={left.heading} items={left.items} />
        <CompareColumn variant="ok" heading={right.heading} items={right.items} />
      </div>
    </section>
  );
}
...
function CompareColumn({
  ...
  const border = isWarn ? "var(--warn)" : "var(--ok)";
  const bg = isWarn ? "var(--warn-soft)" : "var(--ok-soft)";
  const ink = isWarn ? "var(--warn)" : "var(--ok)";
  return (
    <div
      className="rounded-[var(--radius-card-sm)] border p-4"
      style={{ borderColor: border, background: bg }}
    >
      <div className="mb-2.5 flex items-center gap-2">
        <span style={{ color: ink }}>
          {isWarn ? <XCircle size={17} weight="fill" /> : <CheckCircle size={17} weight="fill" />}
        </span>
        <p className="text-[13.5px] font-semibold" style={{ color: ink }}>
          {heading || (isWarn ? "常见误区" : "正确做法")}
        </p>
      </div>
      <ul className="flex flex-col gap-2">
        {items.map((it, i) => (
          <li key={i} className="flex items-start gap-2 text-[13.5px] leading-relaxed text-[var(--ink)]">
            <span className="mt-[7px] h-1.5 w-1.5 shrink-0 rounded-full" style={{ background: border }} />
            <span className="flex-1">{it}</span>
          </li>
        ))}
      </ul>
  ...
  return (
    <section className="overflow-hidden rounded-[var(--radius-card)] border border-[var(--border)] bg-[var(--surface2)] shadow-[var(--card),var(--inner-hi)]">
      ...
      {next && (
        ...
        >
          <span className="text-[11px] font-semibold uppercase tracking-wide text-[var(--ink3)]">下一节</span>
          <span className="flex-1 truncate font-semibold text-[var(--red-ink)]">{next}</span>
          <ArrowRight
            size={16}
            weight="bold"
            className="shrink-0 text-[var(--red-ink)] transition-transform group-hover:translate-x-0.5"
          />
        </a>
      )}
    </section>
